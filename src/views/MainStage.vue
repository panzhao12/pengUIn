...
const penguin = useTemplateRef<SVGSVGElement>('penguin');
const penguinWrapper = useTemplateRef<HTMLDivElement>('penguinWrapper');
const stageRef = useTemplateRef<HTMLDivElement>('stageRef');

// Forbidden zone state — now uses element refs for shape-accurate collision
const showForbiddenOverlay = ref(false);
const forbiddenOverlayRect = ref<{
  x: number;
  y: number;
  width: number;
  height: number;
} | null>(null);
let forbiddenRafId: number | null = null;

// Build forbidden zone descriptors from the actual penguin SVG + piano keys elements
function getForbiddenZones(): { type: 'rect'; el: Element; expand?: number }[] {
  const zones: { type: 'rect'; el: Element; expand?: number }[] = [];
  ...
  const pianoEl = pianoRef.value?.keysEl;
  if (pianoEl) {
    zones.push({ type: 'rect', el: pianoEl, expand: 15 });
  }
...
function handleForbidden(
  ...
  showForbiddenOverlay.value = inZone;
  forbiddenOverlayRect.value = inZone && overlayRect ? overlayRect : null;
}

onUnmounted(() => {
  if (forbiddenRafId !== null) {
    cancelAnimationFrame(forbiddenRafId);
  }
});
...
let currentRecord: NoteRecord | null = null;
const isPaused = ref(false);
let playbackStartLocalTime = 0;
...
function handlePause() {
  ...
  if (activeStickerId.value !== null && !isPaused.value) {
    accumulatedTime += (Date.now() - playbackStartLocalTime) / 1000;
    isPaused.value = true;
  }
}
...
function playRecord(id: number, noteRecord: NoteRecord | undefined) {
  const notes = noteRecord?.record;
  if (!notes || notes.length === 0) return;

  if (activeStickerId.value === id && isPaused.value) {
    isPaused.value = false;
  } else {
  ...
  scheduleNotes(notes, accumulatedTime);
}

function scheduleNotes(notes: any[], offsetTime: number) {
  playbackStartLocalTime = Date.now();
  let maxEndTime = 0;

  for (const note of notes) {
    const safeDuration = Math.min(note.duration, 5);
    const attackTime = note.startTime - offsetTime;
    const releaseTime = note.startTime + safeDuration - offsetTime;

    if (note.startTime + safeDuration > maxEndTime) {
      maxEndTime = note.startTime + safeDuration;
    }

    if (releaseTime <= 0 || attackTime <= 0) continue;

    const attackId = setTimeout(() => {
      sampler.triggerAttack(note.name);
      pianoRef.value?.triggerVisualNote(note.name, safeDuration);
    }, attackTime * 1000);
    playbackTimeouts.push(attackId);

    const releaseId = setTimeout(() => {
      sampler.triggerRelease(note.name);
    }, releaseTime * 1000);
    playbackTimeouts.push(releaseId);
  }

  const remainingTotalTime = maxEndTime - offsetTime;
  if (remainingTotalTime > 0) {
    const stopId = setTimeout(() => {
      if (activeStickerId.value !== null && !isPaused.value) {
        stopPlaybackAndReset();
      }
    }, remainingTotalTime * 1000);
...
const confirmDialog = ref<InstanceType<typeof TheConfirmDialog>>();

function findValidRandomPosition(): { x: number; y: number } {
  const stickerWidth = 100;
  const stickerHeight = 60;
  const maxAttempts = 5000;
  const zones = getForbiddenZones();
  const existingRecords = Array.from(records.value.values());

  for (let i = 0; i < maxAttempts; i++) {
    const x = Math.floor(Math.random() * (window.innerWidth - stickerWidth));
    const y = Math.floor(Math.random() * (window.innerHeight - stickerHeight));
    let valid = true;

    // 1. Check collision with forbidden zones
    for (const zone of zones) {
      const rect = zone.el.getBoundingClientRect();
      const expand = zone.expand || 0;
      if (
        x + stickerWidth > rect.left - expand &&
        x < rect.right + expand &&
        y + stickerHeight > rect.top - expand &&
    ...
    // 2. Check collision with existing stickers
    const buffer = 20; // don't overlap too closely
    for (const r of existingRecords) {
      if (!r.position) continue;

      // Convert existing record positions to pixels if they are in percentages natively
      const rX =
        Math.abs(r.position.x) <= 100
          ? (r.position.x / 100) * window.innerWidth
          : r.position.x;
      const rY =
        Math.abs(r.position.y) <= 100
          ? (r.position.y / 100) * window.innerHeight
          : r.position.y;

      if (
        x + stickerWidth > rX - buffer &&
        x < rX + stickerWidth + buffer &&
        y + stickerHeight > rY - buffer &&
        y < rY + stickerHeight + buffer
      ) {
        valid = false;
        break;
      }
    ...
    if (valid) {
      console.log(`Found valid location after ${i} attempts:`, { x, y });
      return {
  ...
  // fallback to a random position if very crowded
  console.warn(
    'Could not find completely clear spot for sticker. Using raw fallback!'
  );
  const fallbackX = Math.floor(
    Math.random() * (window.innerWidth - stickerWidth)
  );
  const fallbackY = Math.floor(
    Math.random() * (window.innerHeight - stickerHeight)
  );
  return {
    x: (fallbackX / window.innerWidth) * 100,
    y: (fallbackY / window.innerHeight) * 100
  };
}
...
async function handleFormSubmit(input: DialogInput) {
  ...
  const newPosition = findValidRandomPosition();
  await recordStore.saveRecord(input, recordedNotes.value, newPosition);
}

// Deletion logic
function promptDelete(id: number, title: string | null) {
  if (activeStickerId.value === id && !isPaused.value) {
    handlePause();
  }
  confirmDialog.value?.open(id, title);
}
...
<template>
  <div style="margin: auto">
    <TheSticker
      ...
      :is-active="activeStickerId === id"
      :is-paused="activeStickerId === id && isPaused"
      :get-forbidden-zones="getForbiddenZones"
      @play="playRecord(id, record)"
      @pause="handlePause"
      @delete="promptDelete(id, record.title)"
      @forbidden="handleForbidden"
    />

    <div ref="stageRef" class="stage">
      <div ref="penguinWrapper" class="penguin">
        <svg
          ref="penguin"
          ...
          viewBox="0 0 624 624"
          @click="toggleRecord"
          :fill="isRecording ? 'var(--accent-record)' : 'var(--penguin-black)'"
        >
          <path
            ref="penguinPath"
      ...
      <div
        ...
        :class="{ 'recording-indicator--active': isRecording }"
      >
    ...
    <div
      v-if="showForbiddenOverlay && forbiddenOverlayRect"
      class="forbidden-overlay"
      :style="{
        top: forbiddenOverlayRect.y + 'px',
        left: forbiddenOverlayRect.x + 'px',
        width: forbiddenOverlayRect.width + 'px',
        height: forbiddenOverlayRect.height + 'px'
      }"
    />
...
.penguin-jump {
  animation: jump 1.15s ease-in-out infinite;
}
